...
// }
        ...
        if(monthed === (month + 1) && currentDate < birthDate){
    // remainingDays =  (monnthedDays - currentDate) -  (monnthedDays - birthDate)

    remainingDays =  (monnthedDays - birthDate) -  currentDate
...
if(currentDate > birthDate){
            ...
            console.log(Age)
AgedMonth = ((month ) - birthMonth);
RemainingMonth =(12 - birthMonth) + (birthMonth - RemainingMonth );
remainingDays = ((monnthedDays - birthDate) - dateToday);
}
...
// RemainingMonth = birthMonth -  (month + 1 );
if(currentDate < birthDate) RemainingMonth = (monnthedDays > 30) ?(12 - birthMonth) + (birthMonth - RemainingMonth - 1) : (12 - birthMonth) + (birthMonth - RemainingMonth - 1);
...
if(currentDate !== birthDate) remainingDays =  (monnthedDays - birthDate) +   currentDate
...
console.log(monnthedDays)
    return ((birthMonth >= (month + 1)) ?  ` ${Age} : ${RemainingMonth} : ${remainingDays}` : ` ${Age} : ${RemainingMonth} : ${remainingDays}` )
}
console.log(calculateAge(1984, parseInt('9'), parseInt('24')))

//FOR JAN 26 2022 ADD AGED MONTH FOR 1984 OWN ADD REMAINIG MONTH ANF CHANGE REMAINING DAY